...
$$\begin{align} \text{Linear Predictor}:\eta_i = \beta_0 &+ \beta_1(\text{shots}) + \beta_2(\text{timeOnIce}) + \beta_3(\text{isDefenseman}) \\ &+ \beta_4(\text{isLeftWing}) + \beta_5(\text{isRightWing}) + \beta_6(\text{weight}) \\ &+ \beta_7(\text{isCZE}) + \beta_8(\text{isDEU}) + \beta_9(\text{isFIN}) + \beta_{10}(\text{isITA}) \\ &+ \beta_{11}(\text{isLVA}) + \beta_{12}(\text{isPOL}) + \beta_{13}(\text{isRUS}) + \beta_{14}(\text{isSVK}) \\ &+\beta_{15}(\text{isSWE}) +\beta_{16}(\text{isUSA}) + \beta_{17}(\text{hits}) +\beta_{18}(\text{age}) \\ &+ \beta_{19}(\text{faceoffTaken}) \end{align}$$
...
- Holding all other factors constant, the odds of scoring increase by a factor of 1.619792 for every additional shot taken. 

Exponentiated coefficient for USA: 0.7864956
- Holding all other factors constant, the odds of a U.S. player scoring are 0.7864956 times the odds of a Canadien player scoring.

coefficient for RW: 0.3245554
coefficient for D: -0.9205181

Holding all other factors constant, the odds that right wing scores are $e^{0.3245554-(-0.9205181)}=3.473$ times the odds that a defensemen scores. We can also interpret this as the odds for right wing are almost 350% larger than the odds for a defensemen.
...
For example we can perform a prediction on David Pastrnak, who has been a great player for Boston for the current Season. The following are David's variables from a recent game against the Arizona Coyotes that are needed for this model:
...
Inputting these in the model we get:
...
Thus we get: $  \dfrac{\pi_i}{1-\pi_i} = e^{2.40639285} = 11.09$

This means that the odds that David Pastrnak scores a goal in the game are 11.09. The probability that David scores is:

$$\dfrac{e^{2.40639285}}{1+e^{2.40639285}} = 91.73 \%$$

Since one variable that a Coach can control is time on ice, what would be the amount of time David needs to play to have a 75% chance of scoring a goal given some preset values of the following:
...
$$\begin{align} 1.098612289 &= 0.0006238(\text{timeOnIce}) +0.13300105 \\ \text{timeOnIce} &= 1547.95 \text{ secs} \end{align}$$

Thus, David would need to be playing 25 to 26 minutes to have a 75% probability of scoring.
...
From the model and visual we can have some important takeaways that can benefit the Boston Bruin's coaching staff. From the model we see that the number of shots taken and time on ice are two important predictors for whether or not a player scores. Thus, we recommend coaches give their best players opportunities to take more shots and get them on the ice more to increase the chances. Furthermore, from the GLM we see that in the past certain positions such as forwards tend to score more and nationalities like Slovakian and  Russian lead to a higher probability. It interesting to note as well that hits decreases the likelihood of goals. This is due to higher hitting players usually not being goal scorers, thus, a goal scorer is less likely to hit other players.

From the variable importance plot it was interesting to see that takeaways and the hand a player shoots does not matter. Takeaways means that the player took the puck away from another player. We suspected that this would have been a more important variable. We also see that ShootsCatches, is not very important via the variable importance plot.

From the final visuals we see that in the first visual plotting shots and time, and coloring by positions, there are many defensemen who have a lot of time on ice, and a few forwards as well. Furthermore, for those taking a lot of shots, it is almost all forwards. This aligns with what we have interpreted from the GLM. When looking at goals scored by age, we see a larger proportion is scored at younger ages. This makes sense as typically younger players are the great goal scorers. Then for reference to the class we wanted to just make everyone aware of the face offs taken by position. One of the main roles for those playing center is to take face offs. The wings only take face offs when the center is kicked out of the face off. Finally we showed the proportion of goals scored by nationality. What we can see is that some nationalities like Czech and Russian have a higher proportion of goals scored, meaning these players are more likely to be goal scorers.

Future exploration could involve looking at interaction terms, and trying to quantify team chemistry and the players a goal-scorer is playing with can either help or hurt their performance. Furthermore, quantifying the opposing team to get more specific results could improve the accuracy of this analysis.